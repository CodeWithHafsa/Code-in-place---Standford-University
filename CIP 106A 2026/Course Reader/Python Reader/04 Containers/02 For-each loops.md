...
For-each loops work differently on different containers. The only container we have looked at so far is lists, so we'll focus on them for now. With lists, the iterator `element` loops over all the values in the list. You can change the name of `element` to be whatever you like, just as long as the name hasn't been used yet:
...
---
Now, you may be asking yourself, why do we need this? 

After all, didn't we just see how to loop over a list in the last section using regular for loops? For lists, you're right. We can use both notations. 

For-each loops become more necessary with containers that don't have indices like dictionaries (which we will see in the next section), but many programmers still use them with lists because they are considered better style. In general, you only use regular for loops on a list when you need to know the index of the items in the loop.
