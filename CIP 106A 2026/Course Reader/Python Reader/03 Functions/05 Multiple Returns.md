# <center> Multiple Returns </center>

## Quest

Welcome to part 2 of our saga on return statements. 


We know what <span style="color:#4169E1">return</span>  statements are (statements that give back information from the function) and we know how to distinguish them from <span style="color:#4169E1">print</span>   statements. So what is left to learn? 

Well, as the title of this section suggests, it is possible to have multiple return statements in a function. WHAT?! How is this possible? Let’s see below.
...
This code is completely valid. Let’s discuss why. Based on what we know about if statements, any code not in an attached else statement should also run after the body of the if has finished executing (given that the condition was true). 

So why doesn't the second return statement run? As we’ve said before, return statements end the function. This means that if the conditional is true and <span style="color:#4169E1">return</span> num1 runs, everything after that will not be executed. We never get to <span style="color:#4169E1">return</span> num2 even though it is not in an else. 
...
<span style="color:#FFD700">[⚠️ Buggy Code]</span>
