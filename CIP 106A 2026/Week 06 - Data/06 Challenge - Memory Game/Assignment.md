...
This Python programming assignment will guide you through the exciting process of creating your own memory game, a classic and engaging exercise that tests your ability to remember and match pairs of items. In this game the cards will be represented as numbers in a list and their location will be indices of the list. 

By completing this project, you will practice critical list concepts. Not only will this enhance your problem-solving skills, but you will also have a lot of fun as you watch the game come to life and challenge yourself or your friends to find all the pairs.
...
* Here is a demo of a fully implemented solution. Play it so you understand how the game works! [Memory Game Demo](https://codeinplace.stanford.edu/cip5/share/JIGHUopbwtmf2CyQGU8s) 
...
Use a for loop to create a list that has the values  0  through  NUM_PAIRS - 1  twice. So for example if  NUM_PAIRS = 3 as in the example, your list should have this value:
...
3. Determine if they indices have matching values in truth_list
4. If not a match, print the values, "No match. Try again.", and wait for the user to Press Enter. Once they do, call clear_terminal()
5. If it is a match, print "Match!" and call clear_terminal(). (No waiting for the Enter key).

When the game is won:
1. Print out the display array (which will now be fully uncovered)
2. Print 'Congratulations! You won!'
...
def main():
    ...
    while '*' in displayed_list:
        ...
        index1 = get_valid_index(displayed_list, "Enter an index: ")
        index2 = get_valid_index(displayed_list, "Enter an index: ", disallowed=[index1])

        val1 = truth_list[index1]
        val2 = truth_list[index2]

        if val1 == val2:
            # If the two indices are a match, do not print their values at all
            print("Match!")
            displayed_list[index1] = val1
            displayed_list[index2] = val2
        else:
            # If it is not a match, print both index values
